defmodule CustomObjectManagementTest do
  ...
  test "Custom Object Management Integration Test", %{"test_login" => test_login} do
    ...
    if test_object_name == "" do
      ...
      test_ccobu_response = test_login
        |> get_uid()
      ...
      test_ccobt_response = test_login
        |> get_access_token()
        |> LoginRadius.CustomObjectManagement.create_by_access_token(test_object_name, custom_data2)

      assert elem(test_ccobt_response, 0) == :ok

      # Custom Object by ObjectRecordId and UID
      custom_object_id1 = test_ccobt_response
        |> elem(1)
        |> elem(1)
        |> Map.fetch!("Id")

      test_cobou_response = test_login
        |> get_uid()
        |> LoginRadius.CustomObjectManagement.get_by_objectrecordid_and_uid(custom_object_id1, test_object_name)

      assert elem(test_cobou_response, 0) == :ok

      # Custom Object by ObjectRecordId and Token
      test_cobot_response = test_login
        |> get_access_token()
        |> LoginRadius.CustomObjectManagement.get_by_objectrecordid_and_access_token(custom_object_id1, test_object_name)

      assert elem(test_cobot_response, 0) == :ok

      # Custom Object by Token
      test_cot_response = test_login
        |> get_access_token()
      ...
      test_couu_response = test_login
        |> get_uid()
        |> LoginRadius.CustomObjectManagement.update_by_objectrecordid_and_uid(custom_object_id1, test_object_name, "replace", custom_update_data)
      ...
      test_couat_response = test_login
        |> get_access_token()
        |> LoginRadius.CustomObjectManagement.update_by_objectrecordid_and_access_token(custom_object_id1, test_object_name, "partialreplace", custom_update_data2)

      assert elem(test_couat_response, 0) == :ok

      # Delete Custom Object by ObjectRecordId
      test_dcoo_response = test_login
        |> get_uid()
        |> LoginRadius.CustomObjectManagement.delete_by_objectrecordid_and_uid(custom_object_id1, test_object_name)

      assert elem(test_dcoo_response, 0) == :ok

      # Custom Object Delete by Record Id and Token
      custom_object_id2 = test_ccobu_response
        |> elem(1)
        |> elem(1)
        |> Map.fetch!("Id")

      test_codrit_response = test_login
        |> get_access_token()
        |> LoginRadius.CustomObjectManagement.delete_by_objectrecordid_and_access_token(custom_object_id2, test_object_name)

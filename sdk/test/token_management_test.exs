defmodule TokenManagementTest do
  ...
  test "Access Token via Facebook Token" do
    if LoginRadiusTest.facebook_access_token() == "" do
      IO.puts(
        "Need to specify a Facebook access token to test Access Token via Facebook Token API."
      )
    else
      test_atvft_response =
  ...
  test "Access Token via Twitter Token" do
    if LoginRadiusTest.twitter_access_token() == "" &&
      ...
      IO.puts(
        "Need to specify a Twitter access token and token secret to test Access Token via Twitter Token API."
      )
    else
      test_atvtt_response =
  ...
  test "Access Token via Vkontakte Token" do
    if LoginRadiusTest.vkontakte_access_token() == "" do
      IO.puts(
        "Need to specify a Vkontakte access token to test Access Token via Vkontakte Token API."
      )
    else
      test_atvvt_response =
